...
%format _displayListen = listen
%format _displayWriter = writer
%format <> = "\hcmb "
...
%<*freet-heap-ty>
\begin{spec}
type HeapT w m = FreeT ((,) w) (ListT m)
\end{spec}
%</freet-heap-ty>
%<*heap-ty>
\begin{code}
...
type instance Base (HeapT w m a) = HeapF w m a
instance Functor m => Recursive (HeapT w m a) where
  project = coerce
  {-# INLINE project #-}

instance Functor m => Corecursive (HeapT w m a) where
  embed = coerce
  {-# INLINE embed #-}
...
deriving instance (forall x. Eq x => Eq (m x), forall x. Ord x => Ord (m x), Ord a, Ord w) => Ord (HeapT w m a)
...
_use_slow_merge :: ((), Heap () ())
_use_slow_merge = ((), empty) <|||> ((), empty)
...
\begin{code}
(x, xv) <|||> (y, yv)
  | x <= y     = (x, (tell (diff x y) >> yv) <|> xv)
  | otherwise  = (y, (tell (diff x y) >> xv) <|> yv)
\end{code}
%</merge>
%<*tell>
\begin{spec}
tell :: Monad m => w -> HeapT w m ()
...
\end{code}
\begin{spec}
(x,xv) <||> (y,yv)  | x <= y     = (x  , (tell (diff x y) >> yv  ) <|> xv)
                    | otherwise  = (y  , (tell (diff x y) >> xv  ) <|> yv)
\end{spec}
\begin{code}
...
%<*flatten>
\begin{spec}
toCofree :: (Monus w, Monad m) => HeapT w m a -> CofreeT (Maybe . (w,)) m [a]
toCofree = ana popMinT
\end{spec}
%</flatten>
\begin{code}
...
%<*listen>
\begin{code}
_displayListen :: (Functor m, Monus w) => HeapT w m a -> HeapT w m (a, w)
_displayListen xs = ana (HeapF . uncurry (flip (fmap . alg) . runHeapT)) (xs,mempty)
  where
    alg w   (Leaf x)   = Leaf (x, w)
    alg w1  (w2 :< x)  = w2 :< (x, w1 <> w2)
\end{code}
%</listen>
%<*writer>
\begin{code}
_displayWriter :: Monad m => (a, w) -> HeapT w m a
_displayWriter (x, w) = HeapT (pure (w :< pure x))
\end{code}
%</writer>
\begin{code}
...
instance MonadError e m => MonadError e (HeapT w m) where
  ...
  {-# INLINE throwError #-}
  catchError xs h = heapMmap (flip catchError (runListT . runHeapT . h)) xs
  {-# INLINE catchError #-}
